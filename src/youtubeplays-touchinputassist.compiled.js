(function () {
  ...
  var CSS_TO_INJECT = '<style>#touchGridToggle > svg, #touchGridLayout > svg { width: 50%; height: 100% }</style>'; // SCRIPT VARIABLES //
  ...
  function setupVideo() {
    ...
    var touchGridHtml = "<canvas id=\"touchGrid\" width=\"".concat(touchGridSize.width, "\" height=\"").concat(touchGridSize.height, "\" style=\"display: none; z-index: 999; position: absolute; border: 1px solid black; left: ").concat(touchGridSize.left, "%\" />");
    var touchCoordHtml = "<div id=\"touchCoord\" style=\"display: none; z-index: 999; position: absolute; left: ".concat(touchGridSize.left, "%; top: ").concat(touchGridSize.height + 8, "px\">\n<p>Touch Coordinates:</p>\n<p>X: <span id=\"touchCoordX\">0</span></p>\n<p>Y: <span id=\"touchCoordY\">0</span></p>\n<p>Touch Command: <span id=\"touchCoordCommand\">0</span></p>\n</div>");
  ...
  function setupTouchGrid(touchGrid) {
    ...
    touchGrid.on('mousemove', function (e) {
      ...
      $('#touchCoordCommand').text("t:".concat(Math.round(x), ":").concat(Math.round(y)));
    });
    touchGrid.on('click', function (e) {
      ...
      chatInput.focus();
      navigator.clipboard.writeText("t:".concat(Math.round(x), ":").concat(Math.round(y))).then(function () {
        GM_notification({
          title: 'Clipboard',
  ...
  function updateTouchGrid(touchGrid, context, width, height) {
    ...
    touchGrid.css('left', "".concat(touchGridSize.left, "%"));
  ...
  function updateTouchCoord(touchCoord, width, height) {
    ...
    touchCoord.css('left', "".concat(touchGridSize.left, "%"));
    touchCoord.css('top', "".concat(touchGridSize.height + 8, "px"));
